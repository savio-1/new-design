...
.pf-id__meta { display: flex; flex-wrap: wrap; align-items: center; gap: 6px 10px; margin-top: 4px; color: var(--text-teritiary); }
...
/* ── Tabs: three lenses on the same person ── */
.pf-tabs { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
.pf-tabs .cq-seg > button .pf-count { color: var(--text-teritiary); font-weight: 400; }
.pf-pane { display: flex; flex-direction: column; gap: 16px; }
.pf-pane[hidden] { display: none !important; }
.pf-bar { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
.pf-bar__spacer { flex: 1 1 auto; }
...
.ws-mark { width: 32px; height: 32px; border-radius: var(--radius-md); flex: none; display: flex; align-items: center; justify-content: center;
  color: #fff; font: 500 var(--fs-body-2)/1 var(--font-geist, inherit); }
.ws-who__t, .ws-role { display: flex; flex-direction: column; gap: 3px; min-width: 0; }
.ws-who__t .cq-body2-med { color: var(--text-primary); }
.ws-who__t .cq-caption-reg, .ws-role .cq-caption-reg { color: var(--text-teritiary); }
...
/* ── Kind tiles: what a thing is, at a glance ── */
.pf-kind { width: 32px; height: 32px; border-radius: var(--radius-md); flex: none; display: flex; align-items: center; justify-content: center; }
.pf-kind .cq-ic { width: 18px; height: 18px; }
.pf-kind[data-tone="blue"]   { background: var(--backgrounds-badge-blue);   color: var(--text-coloured-blue); }
.pf-kind[data-tone="indigo"] { background: var(--backgrounds-badge-indigo); color: var(--text-coloured-indigo); }
.pf-kind[data-tone="green"]  { background: var(--backgrounds-badge-green);  color: var(--text-coloured-green); }
.pf-kind[data-tone="cyan"]   { background: var(--backgrounds-badge-cyan);   color: var(--text-coloured-cyan); }
.pf-kind[data-tone="orange"] { background: color-mix(in srgb, var(--orange-500) 14%, transparent); color: color-mix(in srgb, var(--orange-600) 78%, var(--text-primary)); }
.pf-kind[data-tone="grey"]   { background: var(--backgrounds-card-bg-4); color: var(--text-secondary); }

/* ── Activity: what this person has done, newest first ── */
.pf-stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; }
.pf-stat { display: flex; flex-direction: column; gap: 4px; padding: 14px 16px; }
.pf-stat .cq-caption-reg { color: var(--text-teritiary); }
.pf-stat__n { color: var(--text-primary); font: 500 22px/28px var(--font-geist); letter-spacing: -0.5px; font-variant-numeric: tabular-nums; }
.pf-stat__d { color: var(--text-teritiary); }
.pf-day { padding: 10px 16px 6px; color: var(--text-teritiary); border-top: 1px solid var(--strokes-line-3); background: var(--backgrounds-page-bg-3); }
.pf-day:first-child { border-top: 0; }
.pf-act { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-top: 1px solid var(--strokes-line-3); }
.pf-act__t { flex: 1 1 auto; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.pf-act__t .cq-body2-reg { color: var(--text-primary); }
.pf-act__t .cq-body2-reg b { font-weight: 500; }
.pf-act__t .cq-caption-reg { color: var(--text-teritiary); }
.pf-act__when { color: var(--text-teritiary); white-space: nowrap; }
.pf-act .cq-badge { flex: none; }

/* ── Published: what this person has put on the marketplace ── */
.pf-pub .cq-card { gap: 10px; }
.pf-pub .cq-card__title-row { align-items: flex-start; gap: 12px; padding: 0; }
.pf-pub .cq-card__t { flex: 1 1 auto; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.pf-pub .cq-card__t .cq-caption-reg { color: var(--text-teritiary); }
.pf-pub .cq-card__desc { white-space: normal; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; line-height: 16px; min-height: 32px; }
.pf-pub__meta { display: flex; align-items: center; gap: 6px; color: var(--text-teritiary); flex-wrap: wrap; }
.pf-pub__meta .cq-ic { width: 12px; height: 12px; color: var(--orange-600); }
.pf-pub__acts { display: flex; align-items: center; gap: 4px; }
.pf-empty { padding: 40px 16px; text-align: center; color: var(--text-teritiary); }

@media (max-width: 1100px) { .pf-grid { grid-template-columns: repeat(2, 1fr); } .pf-kv:nth-child(-n+4) { border-top: 1px solid var(--strokes-line-3); } .pf-kv:nth-child(-n+2) { border-top: 0; } .pf-kv:nth-child(4n) { border-right: 1px solid var(--strokes-line-3); } .pf-kv:nth-child(2n) { border-right: 0; } .pf-stats { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 900px) { .pf-two { grid-template-columns: 1fr; } }
'''

ROW_ICONS = dict(
  lock='<svg class="cq-ic" viewBox="0 0 20 20" fill="none"><rect x="4" y="8.5" width="12" height="8" rx="1.6" stroke="currentColor" stroke-width="1.5"/><path d="M7 8.5V6.5a3 3 0 0 1 6 0v2" stroke="currentColor" stroke-width="1.5"/></svg>',
  shield='<svg class="cq-ic" viewBox="0 0 20 20" fill="none"><path d="M10 2.8 4 5.2v4.6c0 3.6 2.6 6.4 6 7.4 3.4-1 6-3.8 6-7.4V5.2L10 2.8Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><path d="m7.4 10 1.8 1.8 3.4-3.6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  screen='<svg class="cq-ic" viewBox="0 0 20 20" fill="none"><rect x="3" y="4" width="14" height="9.5" rx="1.6" stroke="currentColor" stroke-width="1.5"/><path d="M7 16.5h6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>',
  globe='<svg class="cq-ic" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="7" stroke="currentColor" stroke-width="1.5"/><path d="M3.5 8h13M3.5 12h13M10 3c-2.2 2.2-2.2 11.8 0 14M10 3c2.2 2.2 2.2 11.8 0 14" stroke="currentColor" stroke-width="1.5"/></svg>',
  bell='<svg class="cq-ic" viewBox="0 0 20 20" fill="none"><path d="M5 8.2a5 5 0 0 1 10 0v3.2l1.4 2.1H3.6L5 11.4V8.2Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><path d="M8.2 16a1.9 1.9 0 0 0 3.6 0" stroke="currentColor" stroke-width="1.5"/></svg>',
)
            ...
            <div class="pf-wrap">
              ...
              <section class="ac-card pf-id" aria-label="Who you are">
                ...
                <div class="pf-id__who">
                  ...
                  <div class="pf-id__meta cq-caption-reg" id="pfMeta"></div>
                </div>
                <button class="cq-btn cq-btn--m cq-btn--tonal-1" id="pfEdit" type="button">''' + ICON['pencil'] + '''Edit profile</button>
              </section>

              <div class="pf-tabs">
                <div class="cq-seg" id="pfTabs" role="tablist" aria-label="Profile sections">
                  <span class="cq-seg__thumb" aria-hidden="true"></span>
                  <button data-tab="overview" class="is-active" role="tab" aria-selected="true" aria-controls="pfOverview">Overview</button>
                  <button data-tab="activity" role="tab" aria-selected="false" aria-controls="pfActivity">Activity</button>
                  <button data-tab="published" role="tab" aria-selected="false" aria-controls="pfPublished">Published <span class="pf-count" id="pfPubCount"></span></button>
                </div>
              </div>

              <!-- ── Overview ── -->
              <div class="pf-pane" id="pfOverview" role="tabpanel">
                ...
                <div class="pf-two">
                  <section class="ac-card" aria-label="Security">
                    ...
                    <div class="pf-row"><span class="pf-row__ic">''' + ROW_ICONS['lock'] + '''</span>
                    ...
                    <div class="pf-row"><span class="pf-row__ic">''' + ROW_ICONS['shield'] + '''</span>
                    ...
                    <div class="pf-row"><span class="pf-row__ic">''' + ROW_ICONS['screen'] + '''</span>
                  ...
                  <section class="ac-card" aria-label="Access">
                    ...
                    <div class="pf-row"><span class="pf-row__ic">''' + ICON['key'] + '''</span>
                      <div class="pf-row__t"><span class="cq-body2-med">Personal access tokens</span><span class="cq-caption-reg">19 active · 3 expiring soon · 2 expired</span></div>
                      <button class="cq-btn cq-btn--s cq-btn--tonal-1" id="pfTokens" type="button">Manage''' + ICON['arrow'] + '''</button></div>
                    <div class="pf-row"><span class="pf-row__ic">''' + ROW_ICONS['globe'] + '''</span>
                    ...
                    <div class="pf-row"><span class="pf-row__ic">''' + ROW_ICONS['bell'] + '''</span>
              ...
              <!-- ── Activity ── -->
              <div class="pf-pane" id="pfActivity" role="tabpanel" hidden>
                <div class="pf-stats" id="pfStats"></div>
                <div class="pf-bar">
                  <div class="cq-seg" id="pfActFilter" role="tablist" aria-label="Filter activity">
                    <button data-f="all" class="is-active" role="tab" aria-selected="true">All</button>
                    <button data-f="created" role="tab" aria-selected="false">Created</button>
                    <button data-f="edited" role="tab" aria-selected="false">Edited</button>
                    <button data-f="published" role="tab" aria-selected="false">Published</button>
                    <button data-f="ran" role="tab" aria-selected="false">Runs</button>
                  </div>
                  <span class="pf-bar__spacer"></span>
                  <span class="cq-caption-reg" id="pfActNote" style="color:var(--text-teritiary)"></span>
                </div>
                <section class="ac-card" aria-label="Recent activity"><div id="pfActRows"></div></section>
              </div>

              <!-- ── Published ── -->
              <div class="pf-pane pf-pub" id="pfPublished" role="tabpanel" hidden>
                <div class="pf-bar">
                  <div class="cq-seg" id="pfPubFilter" role="tablist" aria-label="Filter published artifacts">
                    <button data-k="all" class="is-active" role="tab" aria-selected="true">All</button>
                    <button data-k="skill" role="tab" aria-selected="false">Skills</button>
                    <button data-k="assistant" role="tab" aria-selected="false">Assistants</button>
                    <button data-k="automation" role="tab" aria-selected="false">Automations</button>
                    <button data-k="integration" role="tab" aria-selected="false">Integrations</button>
                    <button data-k="context" role="tab" aria-selected="false">Context</button>
                  </div>
                  <span class="pf-bar__spacer"></span>
                  <span class="cq-caption-reg" id="pfPubNote" style="color:var(--text-teritiary)"></span>
                  <button class="cq-btn cq-btn--m cq-btn--primary" type="button" data-toast="Publishing starts from the artifact itself: open it and choose Publish to marketplace">''' + ICON['plus'] + '''Publish to marketplace</button>
                </div>
                <div class="cq-card-grid cq-card-grid--3up" id="pfPubGrid"></div>
              </div>
...
const wsName = id => (WS.find(w => w.id === id) || {}).name || id;

// What things are. One tile per kind, shared by Activity and Published.
const KIND = {
  automation:  { label: 'Automation',  tone: 'blue',   icon: '#ic-pp_automation' },
  assistant:   { label: 'Assistant',   tone: 'indigo', icon: '#ic-pp_aiassistant' },
  skill:       { label: 'Skill',       tone: 'green',  icon: '#ic-pp_skill' },
  integration: { label: 'Integration', tone: 'cyan',   icon: '#ic-pp_integrations' },
  context:     { label: 'Context',     tone: 'orange', icon: '#ic-pp_context' },
  token:       { label: 'Token',       tone: 'grey',   icon: null },
  workspace:   { label: 'Workspace',   tone: 'grey',   icon: '#i-people' },
};
const tile = k => `<span class="pf-kind" data-tone="${KIND[k].tone}">${KIND[k].icon ? `<svg class="cq-ic" aria-hidden="true"><use href="${KIND[k].icon}"/></svg>` : '__KEY__'}</span>`;

/* ── Activity: hours ago, what, on which thing, where ── */
const H = 3600000, NOW = Date.now();
const VERB = {
  created: 'Created', edited: 'Edited', published: 'Published', ran: 'Ran', unlisted: 'Unlisted',
  generated: 'Generated', joined: 'Joined', connected: 'Connected', reviewed: 'Sent for review',
};
const ACTS = [
  [2,    'ran',       'automation',  'Invoice triage',              'finance',   '38 invoices · 2 flagged'],
  [3,    'edited',    'assistant',   'Support triage assistant',    'cs',        'Tightened the escalation prompt'],
  [6,    'published', 'skill',       'PDF summariser',              'design',    'v2.1 · now on the marketplace'],
  [9,    'created',   'context',     'Q3 brand voice',              'marketing', 'From 14 documents in Doc store'],
  [26,   'generated', 'token',       'Release notes bot',           null,        'Personal access token · expires 1 Dec 2026'],
  [29,   'ran',       'automation',  'Weekly digest',               'marketing', 'Sent to 42 people'],
  [31,   'created',   'automation',  'Renewal nudges',              'cs',        'Draft · 6 steps'],
  [50,   'reviewed',  'integration', 'Snowflake connector',         'data',      'Marketplace review · pending'],
  [55,   'edited',    'skill',       'PDF summariser',              'design',    'Fixed table extraction on scanned files'],
  [74,   'connected', 'integration', 'Slack',                       'cs',        'Alerts to #support-triage'],
  [80,   'ran',       'automation',  'Invoice triage',              'finance',   '41 invoices · 0 flagged'],
  [99,   'created',   'assistant',   'Onboarding buddy',            'marketing', 'Answers new-joiner questions'],
  [122,  'published', 'context',     'Brand voice context',         'marketing', 'v1.2 · updated on the marketplace'],
  [150,  'unlisted',  'automation',  'Weekly digest',               'marketing', 'v0.9 · hidden from search, installs keep working'],
  [170,  'joined',    'workspace',   'Data platform',               'data',      'As a viewer'],
  [200,  'edited',    'automation',  'Invoice triage',              'finance',   'Added the flag-for-review step'],
  [260,  'created',   'skill',       'Meeting recap',               'design',    'Summarises a transcript into decisions and owners'],
  [330,  'published', 'assistant',   'Support triage assistant',    'cs',        'v3.0 · now on the marketplace'],
].map(([h, verb, kind, name, ws, note]) => ({ at: NOW - h * H, verb, kind, name, ws, note }));
...
const fmtTime = t => new Date(t).toLocaleTimeString('en-GB', { hour: 'numeric', minute: '2-digit' });
let actFilter = 'all';
function paintActivity() {
  const rows = ACTS.filter(a => actFilter === 'all' || a.verb === actFilter);
  let out = '', last = null;
  rows.forEach(a => {
    const k = dayKey(a.at);
    if (k !== last) { out += `<div class="pf-day cq-caption-med">${fmtDay(a.at)}</div>`; last = k; }
    out += `<div class="pf-act">${tile(a.kind)}
      <div class="pf-act__t">
        <span class="cq-body2-reg">${VERB[a.verb]} ${a.kind === 'workspace' || a.kind === 'token' ? '' : KIND[a.kind].label.toLowerCase() + ' '}<b>${esc(a.name)}</b></span>
        <span class="cq-caption-reg">${esc(a.note)}${a.ws ? ' · in ' + esc(wsName(a.ws)) : ''}</span>
      </div>
      <span class="cq-badge cq-caption-med" data-tone="${KIND[a.kind].tone}">${KIND[a.kind].label}</span>
      <span class="pf-act__when cq-caption-reg">${fmtTime(a.at)}</span>
    </div>`;
  });
  $('pfActRows').innerHTML = out || '<div class="pf-empty cq-body2-reg">Nothing here yet.</div>';
  $('pfActNote').textContent = `${rows.length} of ${ACTS.length} · last 14 days`;
  const n = v => ACTS.filter(a => a.verb === v).length;
  $('pfStats').innerHTML = [
    ['Created', n('created'), 'automations, assistants, skills and context'],
    ['Published', PUBS.length, 'live on the marketplace'],
    ['Runs', n('ran'), 'automations run by hand'],
    ['Workspaces', WS.length, `admin in ${WS.filter(w => w.role === 'Admin').length}`],
  ].map(([l, v, d]) => `<div class="ac-card pf-stat"><span class="cq-caption-reg">${l}</span><span class="pf-stat__n">${v}</span><span class="pf-stat__d cq-caption-reg">${d}</span></div>`).join('');
}
$('pfActFilter').addEventListener('click', e => {
  const b = e.target.closest('[data-f]'); if (!b) return;
  actFilter = b.dataset.f; setSeg($('pfActFilter'), b); paintActivity();
});

/* ── Published: on the marketplace, from this account ── */
const PUBS = [
  { kind: 'skill',       name: 'PDF summariser',           desc: 'Turns long PDFs into a page of findings, with the tables kept intact.',            v: '2.1', installs: 1240, rating: 4.8, status: 'Listed',    ws: 'design',    when: 'Jun 2026' },
  { kind: 'assistant',   name: 'Support triage assistant', desc: 'Reads a ticket, sets priority and routes it, escalating anything about billing.',   v: '3.0', installs: 856,  rating: 4.7, status: 'Listed',    ws: 'cs',        when: 'Feb 2026' },
  { kind: 'automation',  name: 'Invoice triage',           desc: 'Pulls the day\'s invoices, checks them against POs and flags the ones that differ.', v: '1.4', installs: 312,  rating: 4.6, status: 'Listed',    ws: 'finance',   when: 'Nov 2025' },
  { kind: 'context',     name: 'Brand voice context',      desc: 'Tone, vocabulary and the words we never use, for any assistant that writes.',        v: '1.2', installs: 97,   rating: 4.4, status: 'Listed',    ws: 'marketing', when: 'Apr 2026' },
  { kind: 'integration', name: 'Snowflake connector',      desc: 'Read-only access to warehouse tables with row-level policies respected.',            v: '1.0', installs: 0,    rating: null, status: 'In review', ws: 'data',     when: 'Aug 2026' },
  { kind: 'automation',  name: 'Weekly digest',            desc: 'A Monday summary of what changed across the workspace, sent where people read.',      v: '0.9', installs: 41,   rating: 4.1, status: 'Unlisted',  ws: 'marketing', when: 'Sept 2025' },
];
const PUB_TONE = { Listed: 'green', 'In review': 'orange', Unlisted: 'grey' };
let pubFilter = 'all';
function paintPublished() {
  const rows = PUBS.filter(p => pubFilter === 'all' || p.kind === pubFilter);
  $('pfPubGrid').innerHTML = rows.length ? rows.map(p => `<article class="cq-card cq-card--auto">
    <div class="cq-card__title-row">${tile(p.kind)}
      <div class="cq-card__t"><span class="cq-card__title cq-body2-med cq-truncate">${esc(p.name)}</span><span class="cq-caption-reg">${KIND[p.kind].label} · v${p.v} · ${esc(wsName(p.ws))}</span></div>
      <span class="cq-badge cq-caption-med" data-tone="${PUB_TONE[p.status]}">${p.status}</span>
    </div>
    <p class="cq-card__desc">${esc(p.desc)}</p>
    <div class="cq-card__foot">
      <span class="pf-pub__meta cq-caption-reg">${p.installs.toLocaleString('en-GB')} install${p.installs === 1 ? '' : 's'}${p.rating ? ` · <svg class="cq-ic" viewBox="0 0 12 12" aria-hidden="true"><path d="M6 1l1.5 3.2 3.5.5-2.5 2.4.6 3.5L6 8.9 2.9 10.6l.6-3.5L1 4.7l3.5-.5L6 1Z" fill="currentColor"/></svg> ${p.rating}` : ' · not rated yet'} · ${p.when}</span>
      <span class="pf-pub__acts">
        <button class="cq-btn cq-btn--s cq-btn--ghost" type="button" data-toast="Opens ${esc(p.name)} on the marketplace">View</button>
        <button class="cq-btn cq-btn--s cq-btn--outline" type="button" data-toast="Manage ${esc(p.name)}: versions, listing and who can install">Manage</button>
      </span>
    </div>
  </article>`).join('') : '<div class="pf-empty cq-body2-reg">Nothing published of this kind yet.</div>';
  const live = PUBS.filter(p => p.status === 'Listed').length, installs = PUBS.reduce((s, p) => s + p.installs, 0);
  $('pfPubNote').textContent = `${live} listed · ${PUBS.length - live} not yet · ${installs.toLocaleString('en-GB')} installs in all`;
  $('pfPubCount').textContent = PUBS.length;
  wireToasts();
}
$('pfPubFilter').addEventListener('click', e => {
  const b = e.target.closest('[data-k]'); if (!b) return;
  pubFilter = b.dataset.k; setSeg($('pfPubFilter'), b); paintPublished();
});

/* ── Tabs ── */
function cqSegSync(seg) {
  const thumb = seg.querySelector(':scope > .cq-seg__thumb'), active = seg.querySelector(':scope > button.is-active');
  if (!thumb || !active) return;
  thumb.style.width = active.offsetWidth + 'px'; thumb.style.transform = `translateX(${active.offsetLeft - 4}px)`;
}
function setSeg(seg, btn) {
  seg.querySelectorAll(':scope > button').forEach(b => { const on = b === btn; b.classList.toggle('is-active', on); b.setAttribute('aria-selected', String(on)); });
  cqSegSync(seg);
}
const PANES = { overview: 'pfOverview', activity: 'pfActivity', published: 'pfPublished' };
function showTab(tab) {
  Object.entries(PANES).forEach(([k, id]) => { $(id).hidden = k !== tab; });
  try { history.replaceState(null, '', tab === 'overview' ? location.pathname : '#' + tab); } catch (e) {}
}
$('pfTabs').addEventListener('click', e => {
  const b = e.target.closest('[data-tab]'); if (!b) return;
  setSeg($('pfTabs'), b); showTab(b.dataset.tab);
});
...
function paintWs() {
  ...
  const adminIn = WS.filter(w => w.role === 'Admin').length;
...
function paint() {
  ...
  $('pfMeta').innerHTML = `<span>${esc(me.title)} · ${esc(me.team)}</span><span>·</span><span>${esc(me.location)}</span><span>·</span><span>${WS.length} workspaces</span>`;
}
...
if (PANES[want]) { const b = $('pfTabs').querySelector(`[data-tab="${want}"]`); setSeg($('pfTabs'), b); showTab(want); }
