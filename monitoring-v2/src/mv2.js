...
var STATS = [
  ...
  { icon: 'i-gauge',  label: 'Median duration', value: '4.2', unit: 's',
...
function drawVolume() {
  ...
  var padL = 46, padR = 20, padT = 22;
  var volH = 200, gapH = 34, errH = 74, axisH = 22;
  var H = padT + volH + gapH + errH + axisH;
  var plotW = W - padL - padR;

  var svg = el('svg', { viewBox: '0 0 ' + W + ' ' + H, role: 'img',
    'aria-label': 'Executions per hour by automation, with error rate below, last 24 hours' });
  ...
  el('rect', { x: padL - 10, y: padT - 10, width: plotW + 20, height: volH + 20 }, clip);
  ...
  var yV = function (v) { return padT + volH - (v / top) * volH; };

  /* Grid — solid hairlines, one step off the surface. */
  tk.forEach(function (t) {
    el('line', { class: 'mv-gridline', x1: padL, x2: padL + plotW, y1: yV(t), y2: yV(t) }, svg);
    var lb = el('text', { class: 'mv-tick', x: padL - 9, y: yV(t) + 4, 'text-anchor': 'end' }, svg);
    text(lb, fmt(t));
  });

  var hazeG = el('g', { 'clip-path': 'url(#mvVolClip)' }, svg);
  var lineG = el('g', null, svg);
  var labelG = el('g', null, svg);

  SERIES.forEach(function (s) {
    var pts = s.vals.map(function (v, i) { return [x(i), yV(v)]; });
    var d = mono(pts);
    s._d = d; s._pts = pts;
  ...
  /* Direct labels · each series at its own peak. End labels would all
     land in the same 40px at the right edge, where these four lines
     converge; a peak is a distinct x per series, so identity is direct
     without a single label having to be nudged off its own line. */
  var placed = [];
  SERIES.forEach(function (s) {
    var peakV = Math.max.apply(null, s.vals), pi = s.vals.indexOf(peakV);
    var lx = x(pi), ly = yV(peakV) - 11;
    var halfW = s.name.length * 3.3;
    /* Keep it inside the plot and off any label already placed. */
    lx = Math.max(padL + halfW, Math.min(lx, padL + plotW - halfW));
    for (var guard = 0; guard < 4; guard++) {
      var clash = placed.some(function (q) {
        return Math.abs(q.x - lx) < halfW + q.halfW && Math.abs(q.y - ly) < 13;
      });
      if (!clash) break;
      ly -= 14;
    }
    if (ly < padT + 8) ly = padT + 8;
    placed.push({ x: lx, y: ly, halfW: halfW });
    el('circle', { class: 'mv-ring', cx: x(pi), cy: yV(peakV), r: 4, stroke: cat(s.slot) }, labelG);
    var t = el('text', { class: 'mv-peak', x: lx, y: ly, 'text-anchor': 'middle',
      fill: 'var(--text-secondary)', 'font-weight': 500 }, labelG);
    text(t, s.name);
    s._label = t;
  });

  /* ── Error-rate panel · own y axis, same x ── */
  var errTop = padT + volH + gapH;
  var maxE = Math.max.apply(null, ERR);
  var eTop = Math.ceil(maxE / 2) * 2, etk = [];
  for (var ev = 0; ev <= eTop; ev += 2) etk.push(ev);
  var yE = function (v) { return errTop + errH - (v / eTop) * errH; };

  var sep = el('text', { x: padL, y: errTop - 9, class: 'mv-tick', fill: 'var(--text-teritiary)' }, svg);
  text(sep, 'Error rate  ·  %');

  etk.forEach(function (t) {
    if (t === 0) return;
    el('line', { class: 'mv-gridline', x1: padL, x2: padL + plotW, y1: yE(t), y2: yE(t) }, svg);
    var lb = el('text', { class: 'mv-tick', x: padL - 9, y: yE(t) + 4, 'text-anchor': 'end' }, svg);
    text(lb, t);
  });
  el('line', { class: 'mv-baseline', x1: padL, x2: padL + plotW, y1: yE(0), y2: yE(0) }, svg);

  var ePts = ERR.map(function (v, i) { return [x(i), yE(v)]; });
  var eD = mono(ePts);
  /* A single series wants a fill for weight, at a wash — never a
     saturated block. */
  var grad = el('linearGradient', { id: 'mvErrFill', x1: 0, y1: 0, x2: 0, y2: 1 }, defs);
  el('stop', { offset: '0%', 'stop-color': 'var(--st-bad-mark)', 'stop-opacity': .22 }, grad);
  el('stop', { offset: '100%', 'stop-color': 'var(--st-bad-mark)', 'stop-opacity': .02 }, grad);
  el('path', { d: eD + 'L' + x(HOURS - 1) + ',' + yE(0) + 'L' + x(0) + ',' + yE(0) + 'Z',
    fill: 'url(#mvErrFill)', stroke: 'none' }, svg);
  el('path', { class: 'mv-line', d: eD, stroke: 'var(--st-bad-mark)' }, svg);

  /* The one direct label the panel needs: the peak. */
  var peak = ERR.indexOf(maxE);
  el('circle', { class: 'mv-ring', cx: x(peak), cy: yE(maxE), r: 4, stroke: 'var(--st-bad-mark)' }, svg);
  var pl = el('text', { x: x(peak), y: yE(maxE) - 10, 'text-anchor': 'middle',
    fill: 'var(--st-bad)', 'font-weight': 600 }, svg);
  text(pl, pct(maxE));

  /* ── x axis, under the lower panel only ── */
  for (var h = 0; h < HOURS; h += 4) {
    var t2 = el('text', { class: 'mv-tick', x: x(h), y: errTop + errH + 16,
      'text-anchor': h === 0 ? 'start' : 'middle' }, svg);
    text(t2, hourLabel(h));
  }
  ...
  var cLine = el('line', { class: 'mv-crosshair', y1: padT, y2: errTop + errH }, cross);
  var cDots = SERIES.map(function (s) {
    return el('circle', { class: 'mv-ring', r: 4.5, stroke: cat(s.slot) }, cross);
  });
  var cErr = el('circle', { class: 'mv-ring', r: 4, stroke: 'var(--st-bad-mark)' }, cross);

  var hit = el('rect', { class: 'mv-hit', x: padL - 12, y: padT - 8,
    width: plotW + 24, height: volH + gapH + errH + 8 }, svg);

  function at(ev) {
    var box = svg.getBoundingClientRect();
    var scale = W / box.width;
    var px = (ev.clientX - box.left) * scale;
    var i = Math.round(((px - padL) / plotW) * (HOURS - 1));
    return Math.max(0, Math.min(HOURS - 1, i));
  }
  function move(ev) {
    var i = at(ev);
    ...
    SERIES.forEach(function (s, k) {
      cDots[k].setAttribute('cx', x(i)); cDots[k].setAttribute('cy', yV(s.vals[i]));
    });
    cErr.setAttribute('cx', x(i)); cErr.setAttribute('cy', yE(ERR[i]));
    var rows = SERIES.map(function (s) {
      return { color: cat(s.slot), name: s.name, value: fmt(s.vals[i]) };
    });
    rows.push({ color: 'var(--st-bad-mark)', name: 'Error rate', value: pct(ERR[i], 2) });
    var tot = SERIES.reduce(function (a, s) { return a + s.vals[i]; }, 0);
    tipShow(ev.clientX, ev.clientY, hourLabel(i) + ' – ' + hourLabel((i + 1) % 24),
      rows, { name: 'Total runs', value: fmt(tot) });
  }
...
function applyFocus() {
  SERIES.forEach(function (s) {
    ...
    if (s._label) s._label.style.opacity = on ? 1 : 0.3;
  });
...
function renderFailures() {
  ...
  text($('mvFailNote'), fmt(sum) + ' of ' + fmt(totalExec) + ' runs failed today');

  /* Two facts the rows do not carry, in the space under them. */
  var facts = $('mvFailFacts');
  [['26 of ' + fmt(sum), 'retried automatically'],
   ['6.4s', 'median time to failure']].forEach(function (f) {
...
var LIVE = [
  ...
  { nm: 'Doc extraction',   id: 'run_8f21ae', st: 'ok',   dur: 6.7, cost: 0.33, ago: '3m ago',   stages: [.2, .26, .3, .24] },
  { nm: 'Onboarding bot',   id: 'run_8f21a9', st: 'fail', dur: 7.4, cost: 0.31, ago: '4m ago',   stages: [.19, .24, .21, .36] }
];
...
function renderLive() {
  ...
  LIVE.forEach(function (r) {
    ...
    var c6 = html('div', 'mv-cell mv-cell--r', row);
    c6.style.color = 'var(--text-teritiary)';
...
var APPROVALS = [
  ...
  { nm: 'Schema change · orders table', who: 'Doc extraction',  wait: '3m',  tone: 'ok' }
];
...
/* ══ 6 · HEATMAP ══════════════════════════════════════════════════════
   When runs happen: weekday × hour. Level 0 is "no runs" and sits
   outside the ramp — a surface tile, not the ramp's low end. */
...
var HEAT = (function () {
  ...
  for (var d = 0; d < 7; d++) {
    var row = [], weekend = d > 4;
    for (var h = 0; h < 24; h++) {
      var shape = dayShape(h) * (weekend ? 0.28 : 1);
      var v = Math.round(shape * 46 * (0.65 + r() * 0.7));
      row.push(Math.max(0, v));
    }
    grid.push(row);
  }
  /* The afternoon incident, so the heatmap and the error panel tell the
     same story about the same two hours. */
  grid[2][14] = -1; grid[2][15] = -1;
  return grid;
})();

function renderHeat() {
  var dowHost = $('mvHeatDow'), colHost = $('mvHeatCols');
  DOW.forEach(function (d) { text(html('span', null, dowHost), d); });
    ...
    for (var d = 0; d < 7; d++) {
      (function (d, h) {
        ...
        cell.addEventListener('pointerenter', function (ev) {
          tipShow(ev.clientX, ev.clientY, DOW[d] + ' · ' + hourLabel(h),
            v < 0
              ? [{ color: 'var(--st-bad-mark)', name: 'Failed window', value: 'incident' }]
  ...
  /* Scale legend — the ramp is a continuous encoding, so it needs one. */
  var sc = $('mvHeatScale');
