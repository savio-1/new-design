...
let AGENTS = [
  { name: 'Country Rate Checker', type: 'agent', state: 'running',
    ...
    model: 'GPT-4o mini', tools: ['Price list lookup'], skills: ['Compare rate to list', 'Explain a variance'], guards: ['Block quotes outside policy'],
    usedBy: [{ name: 'Quarterly price review', kind: 'automation' }, { name: 'Sales desk assistant', kind: 'assistant' }], by: 'Priya Nair', tags: ['pricing'], updated: '18m ago', runs: 412,
  ...
  { name: 'Spare Parts Stock Planner', type: 'agent', state: 'running',
    ...
    model: 'Claude 3.5 Sonnet', tools: ['Inventory API', 'Demand forecast'], skills: ['Recommend a reorder point', 'Flag slow movers', 'Summarise stock health'], guards: [],
    usedBy: [{ name: 'Weekly stock run', kind: 'automation' }, { name: 'Depot planner assistant', kind: 'assistant' }], by: 'Rahul Menon', tags: ['supply-chain'], updated: '2h ago', runs: 1284 },
  ...
  { name: 'Returns Note Reader', type: 'agent', state: 'error',
    ...
    model: '—', tools: [], skills: [], guards: [],
    usedBy: [], by: 'Marcus Lee', tags: [], updated: '3d ago', runs: 0,
  ...
  { name: 'Spreadsheet Reader', type: 'agent', state: 'idle',
    ...
    model: 'GPT-4o mini', tools: ['File reader'], skills: ['Infer column types'], guards: [],
    usedBy: [{ name: 'Nightly data import', kind: 'automation' }], by: 'Anna Park', tags: ['data'], updated: '1d ago', runs: 96 },
  ...
  { name: 'Document Sorter', type: 'agent', state: 'running',
    ...
    model: 'GPT-4o', tools: ['OCR'], skills: ['Classify a document', 'Score classification confidence'], guards: ['Redact personal data'],
    usedBy: [{ name: 'Inbox triage', kind: 'automation' }, { name: 'Records assistant', kind: 'assistant' }], by: 'Sarah Chen', tags: ['documents'], updated: '42m ago', runs: 2140 },
  ...
  { name: 'Invoice Field Extractor', type: 'agent', state: 'running',
    ...
    model: 'GPT-4o', tools: ['OCR', 'Azure Form Recognizer'], skills: ['Extract header fields', 'Extract line items', 'Normalise totals'], guards: ['Redact personal data'],
    usedBy: [{ name: 'Invoice intake', kind: 'automation' }, { name: 'Accounts payable assistant', kind: 'assistant' }], by: 'Sarah Chen', tags: ['documents', 'azure'], updated: '1h ago', runs: 870 },
  ...
  { name: 'Data Clean-Up Crew', type: 'orchestrator', state: 'running',
    ...
    model: 'GPT-4o', tools: [], skills: ['Reconcile two answers'], guards: ['Stop on disagreement', 'Cap rows per run'],
    usedBy: [{ name: 'Nightly data import', kind: 'automation' }], by: 'Anna Park', tags: ['data'], updated: '20m ago', runs: 318,
  ...
  { name: 'Row Filter', type: 'agent', state: 'idle',
    ...
    model: 'GPT-4o mini', tools: ['Dataset reader'], skills: ['Apply a filter rule'], guards: [],
    usedBy: [{ name: 'Nightly data import', kind: 'automation' }], by: 'Anna Park', tags: ['data'], updated: '20m ago', runs: 318,
  ...
  { name: 'Claims Intake Desk', type: 'orchestrator', state: 'review',
    ...
    model: 'Claude 3.5 Sonnet', tools: ['Claims inbox'], skills: ['Route by claim type', 'Split a mixed submission'], guards: ['Require a claim reference'],
    usedBy: [{ name: 'Claims intake', kind: 'automation' }, { name: 'Adjuster assistant', kind: 'assistant' }], by: 'Mike Torres', tags: ['documents', 'api'], updated: '4h ago', runs: 54,
  ...
  { name: 'API Response Reader', type: 'agent', state: 'running',
    ...
    model: 'GPT-4o', tools: ['HTTP fetch', 'JSON path', 'Schema registry'], skills: ['Name unlabelled fields', 'Flatten nested responses'], guards: ['Never log payloads'],
    usedBy: [{ name: 'Partner sync', kind: 'automation' }], by: 'Sarah Chen', tags: ['api'], updated: '4h ago', runs: 612,
  ...
  { name: 'Results Table Builder', type: 'agent', state: 'draft',
    ...
    model: 'GPT-4o mini', tools: [], skills: ['Lay out a results table'], guards: [],
    usedBy: [], by: 'Marcus Lee', tags: [], updated: '6d ago', runs: 0 },
  ...
  { name: 'Error Log Investigator', type: 'agent', state: 'running',
    ...
    model: 'Claude 3.5 Sonnet', tools: ['Log search', 'Trace lookup'], skills: ['Cluster similar errors', 'Rank by blast radius', 'Draft a likely cause'], guards: ['Never log payloads', 'Read-only access'],
    usedBy: [{ name: 'Incident sweep', kind: 'automation' }, { name: 'On-call assistant', kind: 'assistant' }], by: 'Elena Vogt', tags: ['ops'], updated: '9m ago', runs: 4820 },
  ...
  { name: 'Fraud Risk Scorer', type: 'remote', conn: 'a2a', state: 'running',
    ...
    usedBy: [{ name: 'Payment screening', kind: 'automation' }, { name: 'Risk desk assistant', kind: 'assistant' }] },
  ...
  { name: 'Market Research Assistant', type: 'remote', conn: 'a2a', state: 'idle',
    ...
    usedBy: [{ name: 'Competitor digest', kind: 'automation' }] },
  ...
  { name: 'Weather Lookup', type: 'remote', conn: 'rest', state: 'running',
    ...
    usedBy: [{ name: 'Route planning', kind: 'automation' }, { name: 'Depot planner assistant', kind: 'assistant' }] },
  ...
  { name: 'Delivery Tracker', type: 'remote', conn: 'rest', state: 'error',
    ...
    usedBy: [{ name: 'Delivery watch', kind: 'automation' }],
...
const MARKET = [
  { name: 'Contract Risk Reviewer', type: 'agent', state: 'idle',
    ...
    model: 'Claude 3.5 Sonnet', tools: ['Clause library', 'Redline diff', 'Document store'], skills: ['Find a clause', 'Compare to standard', 'Rate a risk', 'Draft a redline'], guards: ['Redact personal data', 'Legal review required'],
    usedBy: [], by: 'Legal Ops', tags: ['legal', 'documents'], updated: '3d ago', runs: 0, installs: 42 },
  { name: 'Unusual Spend Spotter', type: 'agent', state: 'idle',
    ...
    model: 'GPT-4o', tools: ['Ledger query', 'Vendor registry'], skills: ['Detect an outlier', 'Explain a variance'], guards: ['Read-only access'],
    usedBy: [], by: 'Finance Guild', tags: ['finance'], updated: '1w ago', runs: 0, installs: 118 },
  { name: 'Supplier Problem Room', type: 'orchestrator', state: 'idle',
    ...
    model: 'GPT-4o', tools: ['Supplier registry'], skills: ['Summarise an incident', 'Draft a supplier note', 'Track a resolution'], guards: ['Stop on disagreement', 'Require a human sign-off'],
    usedBy: [], by: 'Supply Chain CoE', tags: ['supply-chain', 'ops'], updated: '2w ago', runs: 0, installs: 67,
  ...
  { name: 'Support Ticket Sorter', type: 'agent', state: 'idle',
    ...
    model: 'GPT-4o mini', tools: ['Ticket queue', 'Knowledge base'], skills: ['Classify a ticket', 'Suggest a reply'], guards: ['Redact personal data'],
    usedBy: [], by: 'Support Guild', tags: ['ops'], updated: '4d ago', runs: 0, installs: 203 },
  { name: 'Invoice Matcher', type: 'agent', state: 'idle',
    ...
    model: 'GPT-4o mini', tools: ['Ledger query', 'Purchase orders', 'OCR'], skills: ['Match invoice to PO', 'Explain a mismatch'], guards: ['Require a human sign-off'],
    usedBy: [], by: 'Finance Guild', tags: ['finance', 'documents'], updated: '5d ago', runs: 0, installs: 91 },
...
function capsHTML(a) {
  ...
  /* The chips count; the panel names them. Both read the same arrays. */
  const chip = (list, icon) => list && list.length
    ? `<span class="ag-cap" title="${esc(list.join(', '))}">${ic(icon, 14)}${list.length}</span>` : '';
  out.push(chip(a.tools, 'i-wrench', 'tool'));
  out.push(chip(a.skills, 'ic-pp_skill', 'skill'));
  out.push(chip(a.guards, 'ic-pp_guardrails', 'guardrail'));
  return out.join('');
}
...
function cardHTML(a, i) {
  ...
  <article class="cq-card ag-card cq-tint" tabindex="0" role="button" data-i="${i}"
    ...
    ${a.type === 'agent' ? '' : `<span class="ag-card__kind cq-caption-med">${esc(TYPE[a.type].label)}</span>`}
